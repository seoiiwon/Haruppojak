...
template_calendar = os.path.join(os.path.dirname(
    __file__), "../../Web/templates/CalendarPage")
...
async def writediarys(writediary: CreateDiarySchema, currentUser: AuthSchema.UserInfoSchema = Depends(getCurrentUser), db: Session = Depends(get_db)):
    ...
    latest_diary = db.query(UserDiary).filter(
        UserDiary.Diaryuserid == userid, UserDiary.Date.like(f'{target_date}%')).first()
    if latest_diary:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST,
                            detail="뽀짝일기는 하루에 한 번만 작성할 수 있습니다. 내일 또 만나요 !")
